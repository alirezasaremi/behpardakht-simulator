...
Goal 3 implements only safe table-1 normal-path extraction. Table marks `mobileNo`, `encPan`, `panHiddenMode`, `cartItem`, and `enc` optional. It marks `enc` required only for documented Mana brokerage use; Goal 3 has no merchant-category configuration and cannot determine that condition. Local SOAP rejects `mobileNo`, `encPan`, and `enc` before application handling to prevent personal/card/identity-data intake; it accepts then discards `panHiddenMode` and `cartItem`. This is `SIMULATOR_INTERNAL`, not a provider acceptance claim. Source gives `additionalData` maximum 1000 characters and syntax examples `YYYYMMDD` / `HHMMSS` for `localDate` / `localTime`.
