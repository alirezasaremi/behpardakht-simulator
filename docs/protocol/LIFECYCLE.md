...
Goal 8 accepts combined VerifySettle for correlated successful Sale. Its `0` atomically records `VERIFIED` and `REQUESTED`, with one `SETTLEMENT_REQUESTED` event marked `via: "VERIFY_SETTLE"`, never separate Verify/Settle SOAP history. Direct VerifySettle retry prose plus table 11 establishes `43` after separate Verify, `45` after settlement request, and `48` for known reversal; no such result changes state. Non-success Sale and unsupported states fault locally. No callback, automatic reversal, timer, or financial settlement follows this operation.

Goal 9 adds only `SIMULATOR_SCENARIO` reachability, never protocol mechanism. `VERIFY_UNRESOLVED` stores repeated `VERIFY_ATTEMPTED` state, returns local scenario Fault not numeric provider result. Clear leaves history, permits later normal Verify. `KNOWN_REVERSED` has simulator-safety eligibility (successful Sale, unresolved Verify, no settlement); records `SCENARIO_STATE_FORCED`, not `bpReversalRequest`. Once state known `REVERSED`, existing `PROTOCOL` Verify/VerifySettle mappings return `48`. Scenario control transaction-local process memory; clear never undoes state.
