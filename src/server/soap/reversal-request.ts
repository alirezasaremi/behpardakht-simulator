import type { BpReversalRequestInput } from "@/server/protocol";
...
/** PROTOCOL: v1.39 printed page 24 table 5 fields; local checks are SIMULATOR_INTERNAL. */
export function extractBpReversalRequest(operation: ParsedSoapOperation): BpReversalRequestInput {
  ...
  for (const name of operation.fields.keys()) {
    if (!ALLOWED_FIELD_NAMES.has(name)) {
      throw new SoapInputError("INVALID_REVERSAL_REQUEST", "bpReversalRequest contains an unsupported field.");
    }
  }
...
function required(operation: ParsedSoapOperation, name: (typeof REQUIRED_FIELD_NAMES)[number]): string {
  ...
  if (value === undefined) {
    throw new SoapInputError("INVALID_REVERSAL_REQUEST", "bpReversalRequest is missing a required field.");
  }
  return value;
}
...
function decimalBigInt(name: string, value: string): bigint {
  if (!/^\d+$/.test(value)) {
    throw new SoapInputError("INVALID_REVERSAL_REQUEST", `${name} must be a decimal integer.`);
  }
  return BigInt(value);
}
...
function assertMaximumLength(name: string, value: string): void {
  if (Array.from(value).length > MAX_LOCAL_STRING_CHARACTERS) {
    throw new SoapInputError("INVALID_REVERSAL_REQUEST", `${name} exceeds a permitted size.`);
  }
}
