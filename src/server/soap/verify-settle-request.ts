import type { BpVerifySettleRequestInput } from "@/server/protocol";
...
/** PROTOCOL: v1.39 printed pages 31-32 table 12; structural checks are SIMULATOR_INTERNAL. */
export function extractBpVerifySettleRequest(operation: ParsedSoapOperation): BpVerifySettleRequestInput {
  ...
  for (const name of operation.fields.keys()) {
    if (!ALLOWED_FIELD_NAMES.has(name)) {
      throw new SoapInputError("INVALID_VERIFY_SETTLE_REQUEST", "bpVerifySettleRequest contains an unsupported field.");
    }
  }
...
function required(operation: ParsedSoapOperation, name: (typeof REQUIRED_FIELD_NAMES)[number]): string {
  ...
  if (value === undefined) {
    throw new SoapInputError("INVALID_VERIFY_SETTLE_REQUEST", "bpVerifySettleRequest is missing a required field.");
  }
  return value;
}
...
function decimalBigInt(name: string, value: string): bigint {
  if (!/^\d+$/.test(value)) {
    throw new SoapInputError("INVALID_VERIFY_SETTLE_REQUEST", `${name} must be a decimal integer.`);
  }
  return BigInt(value);
}
...
function assertMaximumLength(name: string, value: string): void {
  if (Array.from(value).length > MAX_LOCAL_STRING_CHARACTERS) {
    throw new SoapInputError("INVALID_VERIFY_SETTLE_REQUEST", `${name} exceeds a permitted size.`);
  }
}
